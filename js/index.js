    ...
    recettes.forEach((recette) => {
        ...
        tab.forEach((motCles) => {
            ...
            recette.ingredients.forEach((el) => {
                if (
                    el.ingredient.toLowerCase().includes(motCles.toLowerCase())
                ) {
                    motClesPresent = true
                    console.log(
                        'première verrification ',
                        motClesPresent,
                        motCles
                    )
                }
        ...
        if (nbTrue === tab.length) {
            console.log(
                'ils sont ==> ',
                nbTrue,
                ' recettes a être inclure les mots clés'
            )
    ...
    if (e.target === barreRecherche) {
        ...
        if (saisie.length > 3) {
            if (newRecettes.length > 0) {
                ...
                removeGalerie()
                galerie.textContent =
                    'Aucun élément de la recherche ne correspond'
            }
        } else {
            if (store < 1) {
                removeGalerie()
...
async function trieRechercheInput() {
   ...
   */
            const newTab = tab.filter((el) =>
                el.toLowerCase().includes(valeurSaisie)
            )
